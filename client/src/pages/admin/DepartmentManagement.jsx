...
const DepartmentManagement = () => {
  // Super Admin: fetch and select organization to scope departments
  ...
  const fetchDepartments = async () => {
    // Only load departments once an organization is selected
    if (!selectedOrg) {
      setDepartments([]);
      setDepartmentTree([]);
      return;
    ...
    try {
      ...
      const [deptResponse, hierarchyResponse, usersResponse] =
        await Promise.all([
          axios.get(`/api/departments?organizationId=${selectedOrg}`),
      ...
      if (hierarchyResponse.data.success) {
        // Filter full hierarchy down to selected organization
        const filteredTree = hierarchyResponse.data.data.filter(
          (dept) => dept.organizationId === selectedOrg
        );
        setDepartmentTree(filteredTree);
      }
  ...
  // Re-fetch when organization selection changes
  useEffect(() => {
    fetchDepartments();
  }, [selectedOrg]);
    ...
    if (user.role === "SUPER_ADMIN") {
      axios.get("/api/organizations").then(res => {
        ...
        notification.error({
          ...
          description: err.response?.data?.message || "Could not fetch organizations",
          placement: "bottomRight",
        });
      });
  ...
  // Render organization selector for Super Admin
  return (
    <div>
